...
Queue.prototype._findCommit = function _findCommit(items, needle) {
  var arr = [];
  if (!items) return;
  var keys = Object.keys(items);
  for (var i = 0, item, c; item = items[keys[i]]; i++) {
    if (item.activeWorker === this._workerId) continue;
    c = Commit.from(item);
    if (_.isEqual(c, needle)) {
      arr.push(item);
    }
  }
  return arr;
...
Queue.prototype.remove = function remove(commit, done) {
  ...
  if (currentlyActive.length) {
    ...
    this._itemsSnapshot.ref().transaction(function(items) {
      var commitsInQueue = this._findCommit(items, commitNeedle);
      _.each(commitsInQueue, function(commit) {
        this._fbQueue.child(commit).remove();
      }.bind(this));
    }.bind(this), function(error, committed, snapshot) {
      if (error) {
