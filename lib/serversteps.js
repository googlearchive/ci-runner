...
function startServer(config, workerLog, queue, fbRoot, github, done) {
  ...
  app.use(function(req, res, next){
    workerLog.info('%s %s', req.method, req.url);
    next();
  });
  ...
  app.get('/test', function(req, res) {
    ...
    if (!user || !repo || !ref) {
      res.send('Invalid commit identifier. Expected <user>/<repo>[@ref]');
      return;
    }

    res.write('Looking up commit... ');
  ...
  hooks.on('push', function(event) {
    workerLog.info('Received GitHub push event:', event);
  ...
  hooks.on('pull_request', function(event) {
    workerLog.info('Received GitHub pull_request event:', event);
    var payload = event.payload;
