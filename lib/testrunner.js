...
TestRunner.prototype._runTests = function _runTests(next) {
  ...
  var reporter = runner(false, browsers, options, next);

  ['log', 'run-start', 'run-end', 'browser-start', 'browser-end', 'test-status'].forEach(function(event) {
    reporter.on(event, function() {
      this._log.info(chalk.yellow(event), Array.prototype.slice.apply(arguments, 1));
    }.bind(this));
  }.bind(this));
};
