...
TestMonitor.prototype.setCommitStatus = function setCommitStatus(state, text, done) {
  ...
  var cleanText = chalk.stripColor(text);

  this._log.info('Setting commit status to', chalk.yellow(state) + ':', chalk.blue(text));
  this._commitStatus = {state: state, text: cleanText};
...
TestMonitor.prototype._onBrowserEnd = function _onBrowserEnd(browserInfo, error, stats) {
  this._getBrowser(browserInfo).update(stats, error);
  this._emitShortStatus();
};
...
TestMonitor.prototype._emitShortStatus = function _emitShortStatus() {
  ...
  for (var i = 0, key; key = keys[i]; i++) {
    if (this._browsers[key].stats.failing > 0 || this._browsers[key].error) {
      failing = failing + 1;
    } else if (this._browsers[key].status === 'complete') {
      passing = passing + 1;
    }
  }
  var pending = keys.length - failing - passing;

  var tokens  = [];
  if (failing > 0) { tokens.push(failing  + ' failing'); }
  if (passing > 0) { tokens.push(passing  + ' passing'); }
  if (pending > 0) { tokens.push(complete + ' pending'); }

  var status = failing > 0 ? 'failing' : (pending > 0 ? 'pending' : 'success');
  this.setCommitStatus(status, tokens.join(', ') + ' browsers');
};
