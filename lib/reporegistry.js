...
RepoRegistry.prototype._syncMirror = function _syncMirror(commit, dest, mirror, log, next) {
  synchro(commit.repoUrl, function(done) {
    ...
    // Fresh mirror?
    if (!fs.existsSync(path.join(mirror, 'config'))) {
      async.series([
        mkdirp.bind(null, mirror),
        exec.bind(null, log, 'git', ['clone', '--mirror', commit.repoUrl, '.'], mirror),
      ], done);

    // Already mirrored; just fetch updates.
    } else {
      exec(log, 'git', ['fetch', 'origin'], mirror, done);
    }
  }, next);
};
