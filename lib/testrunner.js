...
var NOBODY_UID = userid.uid('nobody');
var NOBODY_GID = (function(possibles) {
  var gid = -1;
  for (var i = 0; i < possibles.length; i++) {
    try {
      gid = userid.gid(possibles[i]);
      break;
    } catch(_) {}
  }
  return gid;
...
TestRunner.prototype._disownSandbox = function _disownSandbox(next) {
  if (this._cancelled) return next('cancelled');
  var userid = require('userid');
  fs.chown(this._sandbox, NOBODY_UID, NOBODY_GID, next);
};
...
TestRunner.prototype._bowerInstall = function _bowerInstall(next) {
  if (this._cancelled) return next('cancelled');
  // run bower install as 'nobody'
  var spawnArgs = {
    cwd: this._root,
    uid: NOBODY_UID,
    gid: NOBODY_GID
  };
  exec(this._log, 'bower', ['install'], spawnArgs, next);
};
