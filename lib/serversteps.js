...
function establishSauceTunnel(config, workerLog, done) {
  ...
  // handle interrupts to kill the sauce tunnel
...
function startServer(config, workerLog, queue, fbRoot, github, done) {
  ...
  app.get('/slack/kill', function(req, res) {
    ...
    console.log("Equal?", query.channel_id, config.slackChannelId, query.channel_id === config.slackChannelId);
    if (query.channel_id === config.slackChannelId) {
      res.write('CI runner is rebooting...');
      require('cleankill').interrupt(function(){});
    }
    res.end();
  });
