...
TestRunner.prototype.cancel = function cancel() {
  if (this._reporter) {
     ...
     * wct uses cleankill to handle interrupts to tests
     * use that functionality to kill the current running browser
     */
    require('cleankill').close(function(){
      this._reporter = null;
    }.bind(this));
  }
};
...
TestRunner.prototype._runTests = function _runTests(next) {
  ...
  // fail tests if runner takes > itemTimeout in config
  timer = setTimeout(function() {
    finish(new Error('Maximum Test Timeout Exceeded!'));
  }, this._config.worker.maxTestRunnerTimeout);
