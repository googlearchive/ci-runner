...
var TRAVIS_FAILURE = 'Found .travis.yml, not running tests';
...
TestRunner.prototype.run = function run(done) {
  ...
  async.series([
    ...
    this._cleanup(function() {
      ...
      // quitting on .travis.yml is not a real failure
      if (error === TRAVIS_FAILURE) {
        return done();
      } else {
        done(error);
      }
...
TestRunner.prototype._fetchRepo = function _fetchRepo(next) {
  ...
  this._repos.clone(this._commit, this._root, this._log, function(error) {
    if (error) return next(error);
    if (fs.existsSync(path.join(this._root, '.travis.yml'))) {
      return next(TRAVIS_FAILURE);
    }
