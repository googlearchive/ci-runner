...
Queue.prototype.remove = function remove(commit, done) {
  ...
  if (!this._itemsSnapshot) return done('Queue empty');

  this._removing = true;
  console.log('Removing', commit, 'from the queue');
  // all we care about is user/repo@sha
  var commitNeedle = {
    user: commit.user,
    repo: commit.repo,
    sha: commit.sha
  ...
  this._itemsSnapshot.ref().transaction(function(items) {
    ...
    if (keys.length > 0) {
      for (var i = 0, item, c; item = items[keys[i]]; i++) {
        console.log('Equal:', item, commitNeedle);
        // use lodash filter as a fuzzy equality check
        if (_.filter([item], commitNeedle).length > 0) {
    ...
    if (error || !committed) {
      console.log('Remove failed for', commit, 'committed:', committed, 'error:', error);
      done(error);
    } else {
