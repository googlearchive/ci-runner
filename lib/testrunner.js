...
TestRunner.prototype.run = function run(done) {
  async.series([
    ...
    this._cleanup(function() {
      this.setCommitStatus(failure ? 'failed' : 'success', this._commitStatus.text);
...
// TODO(nevir): https://github.com/PolymerLabs/ci-runner/issues/22
...
TestRunner.prototype._setTestStatus = function _setTestStatus() {
  var message = this._numPlatforms + ' platforms: '
              + '✔' + this._numPassed + ', '
              + '✗' + this._numFailed;
  var state = this._numFailed > 0 ? 'failed' : 'pending';
  this.setCommitStatus(state, message);
};
