...
function Config(env) {
  ...
  this.worker = {
    ...
    itemTimeout: env.ITEM_TIMEOUT || 30 * 60 * 1000, // 30 minutes.
    // List of allowed test runners.
    validRunners: (env.VALID_RUNNERS || 'web-component-tester').split(','),
    // List of branches to listen to pushes for.
    pushBranches: (env.PUSH_BRANCHES || 'master').split(','),
    // How frequently the worker should try updating runners.
    runnerRefreshInterval: 86400000, // 1 day.
    // max test runner time
    maxTestRunnerTimeout: env.MAX_TEST_RUNNER_TIMEOUT || 30 * 60, // 30 minutes (in seconds)
    // slack channel group id
  ...
  this.email = {
    ...
    // Extra recipients (BCC'd).
    recipients: env.NOTIFICATION_RECIPIENTS,
